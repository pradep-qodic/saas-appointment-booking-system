...
$(document).ready(function() {
	//loadResources();
	if($("select#resourceID option:selected").text()!==""){
		loadUserCalData($("select#resourceID option:selected").val());
		$('#luserID').val($("select#resourceID option:selected").val());
		$('#uID').val($("select#resourceID option:selected").val());
	}
	else{
		$("#resourceInfo").html("<h2 class='text-center'>There is no resource calendar found.</h2>");
	}
            ...
            if(data.status=="success"){
				...
				if(res.length>0){
					...
					for(var i=0;i<res.length;i++){
						$("#resourceID").append('<option value="'+res[i]['org_user_id']+'">'+res[i]['name']+'</option>');
					}
					$("#uID").val($("select#resourceID option:selected").val());
					$('#leaveCalendar').fullCalendar('destroy');
					loadUserCalData($("select#resourceID option:selected").val());
...
calendarEventClick = function(event, jsEvent, view) {
		if(event.id){
			...
			loadUserWorkingPlan(y+"-"+(m+1)+"-"+d,$("select#resourceID option:selected").val());
		}
}