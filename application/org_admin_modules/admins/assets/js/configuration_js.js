$(document).ready(function() {
	...
	$("#rightCol").on("click","#redirectionUrl",function(){
		$("#checkRedirect").prop("disabled",false);
		$("#checkRedirect").prop("checked",true);
		$("#redirectVal").prop("disabled",false);
	});
	$("#btnUsernotification").click(function(){
		saveUserNotifications();
	});
	$('#duplicateBookingPage').click(function () {
		$("#bookingPageId").val($("#resource_id").val()); 
	});
	...
	$.ajax({
        ...
        success: function(data){
            if(data.status=="success"){
				...
				$("#bookingPageOwner").html(data.data.pageInfo[0].firstname);
				if(data.data.pageInfo[0].redirectionSec && data.data.pageInfo[0].redirectionSec !='-1'){
					$("#redirectVal").val(data.data.pageInfo[0].redirectionSec);
					$("#checkRedirect").prop("checked",true);
					$("#redirectionUrl").val(data.data.pageInfo[0].redirectionUrl);
				}else{
					$("#checkRedirect").prop("checked",false);
					$("#redirectVal").prop("disabled",true);
					$("#checkRedirect").prop("disabled",true);
					$("#redirectionUrl").val("");
				}
	...
	$.ajax({
        ...
        success: function(data){
            if(data.status=="success"){
				...
				if(data.data.applicantNotifications.followupEmailVal !="none"){
				...
				var rem0,rem1,rem2;
				if(reminderVal && reminderVal[0] !="none"){
					rem0=reminderVal[0];
					$("#reminderVal0").val(rem0);
					$("#checkReminder0").prop("checked",true);
				}else{
					rem0="";
					$("#reminderVal0").prop("disabled",true);
					$("#checkReminder0").prop("checked",false);
				}
				if(reminderVal && reminderVal[1] !="none"){
					rem1=reminderVal[1];
					$("#reminderVal1").val(rem1);
					$("#checkReminder1").prop("checked",true);
				}else{
					rem1="";
					$("#reminderVal1").prop("disabled",true);
					$("#checkReminder1").prop("checked",false);
				}
				if(reminderVal && reminderVal[2] !="none"){
					rem2=reminderVal[2];
					$("#reminderVal2").val(rem2);
					$("#checkReminder2").prop("checked",true);
				}else{
					rem2="";
					$("#reminderVal2").prop("disabled",true);
					$("#checkReminder2").prop("checked",false);
				}
...
function saveBooking(){
	...
	var sec='-1';
	var id=$("#resource_id").val();
	if($('#checkRedirect').is(":checked")){
		var rUrl=$("#redirectionUrl").val();
		var sec=$("#redirectVal").val();
	}
	if($("#redirectionUrl").val()==''){
		var rUrl='';
		var sec='-1';
	}
	...
	$.ajax({
        type:"post",
        data:{pageId:id,bookingAuto:bookingVal,redirectionSec:sec,redirectionUrl:rUrl},
