	...
	$.ajax({
        ...
        success: function(data){
			...
			if(data.data.slots.length>0){
				$("#orgslots").append("<b><h4 style='margin-bottom:20px;font-size: 17px;' id='sDate' dt='"+data.data.selectedDate+"'>Availability for "+data.data.availability+"</h4></b>");
...
function fullCal(){
	...
	$('#calendar').fullCalendar({
		...
		dayClick: function(date, jsEvent, view) {
			if(new Date(date.format()) >= new Date(y+"-"+(m+1)+"-"+d) && $('.fc-day[data-date="' + date.format() + '"]').attr("holiday") !="true"){
...
function loadCalAppointments(start, end, timezone, callback) {
	var id=$("#pageId").val();
	$.ajax({
		url: $("#current_url").val()+'/loadAppointments',
		type:"post",
		data:{pageId:id},
		success: function(data) {
