	...
	$('#calendar').fullCalendar({
		...
		dayClick: function(date, jsEvent, view) {
			...
			if(new Date(date.format()) >= new Date(y+"/"+(m+1)+"/"+d) && $('.fc-day[data-date="' + date.format() + '"]').attr("holiday") !="true" && $('.fc-day[data-date="' + date.format() + '"]').attr("weekends") !="true"){
				...
				if(isUnavai=='true')
					unavailability='<div><button id="btnSetUnavailable" data-dt="'+date.format()+'" class="btn btn-primary">Mark as unavailable</button></div>';
			...
			if($('.fc-day[data-date="' + date.format() + '"]').attr("holiday") =="true"){
				...
				var isUnavai=$("#allowSetUnavailability").val();
				if(isUnavai=='true')
					unavailability='<div class="alert alert-success alert-dismissible" role="alert" id="markAvailableMsgSuccess" style="display:none;">'+
					'<button type="button" class="close" data-dismiss="alert"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>'+
					'<strong>Success !!!</strong> <span id="markAvailableMsg"></span>'+
					'</div><div><button id="btnSetAvailable" data-id="'+hId+'" class="btn btn-danger">Mark as available</button></div>';
				var popupHTML='<div><a href="#" class="timesClass"><span class="fa fa-times pull-right"></span></a><br>'+
		...
		eventClick: function(calEvent, jsEvent, view) {
			...
			$("#date").val('');
			$("#dateAvailableSlots").html('Select date for slots');
...
function loadCalConfirmAppointments(start, end, timezone, callback) {
	...
	if(showAppointments=='true'){
		$.ajax({
			url: $("#current_url").val()+'/loadPageAppointments/Confirm',
			type:"post",
			beforeSend: function (xhr) {
				xhr.setRequestHeader ("Authorization", "Basic "+token);
			},
...
function loadCalCancelAppointments(start, end, timezone, callback) {
	...
	if(showAppointments=='true'){
		$.ajax({
			url: $("#current_url").val()+'/loadPageAppointments/Cancel',
			type:"post",
			beforeSend: function (xhr) {
				xhr.setRequestHeader ("Authorization", "Basic "+token);
			},
...
function loadCalPendingAppointments(start, end, timezone, callback) {
	...
	if(showAppointments=='true'){
		$.ajax({
			url: $("#current_url").val()+'/loadPageAppointments/Pending',
			type:"post",
			beforeSend: function (xhr) {
				xhr.setRequestHeader ("Authorization", "Basic "+token);
			},
...
function markAvailable(id){
	var token=$("#authToken").val();
	$.ajax({
		url: $("#current_url").val()+'/markAvailable',
		type:"post",
		beforeSend: function (xhr) {
			xhr.setRequestHeader ("Authorization", "Basic "+token);
		},
